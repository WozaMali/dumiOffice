-- Personalisation flow for storefront /personalisation
-- *** RUN THIS ENTIRE FILE ONCE in Supabase → SQL Editor → Run ***
-- If a previous run failed partway, use docs/SUPABASE_PERSONALISATION_REPAIR.sql instead.
-- Until you run it, Office will error: "Could not find personalisation_settings in the schema cache"
--
-- Office manages:
--   - Fee, per-category blank bottle previews (mens / womens / unisex / diffuser)
--   - Label position, placeholder text, four font options
--
-- Storefront reads these tables (anon SELECT on active rows).
-- Upload blank bottle to hero-assets bucket, e.g. personalisation/blank-bottle-photo.jpg

-- ---------------------------------------------------------------------------
-- Settings (one row per code; use code = 'default')
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Font options (4 choices on personalisation page)
-- ---------------------------------------------------------------------------
...
-- updated_at triggers (reuse set_updated_at if it exists from front_popups)
...
-- Storefront: read active settings
...
-- ---------------------------------------------------------------------------
-- Repair partial / older installs (MUST run before seed INSERT)
...
-- Deactivate legacy demo fonts if present
...
-- ---------------------------------------------------------------------------
-- After uploading blank bottles to hero-assets (one per step-1 category):
-- ---------------------------------------------------------------------------
-- update public.personalisation_settings
-- set
--   preview_image_mens = 'personalisation/mens/blank-bottle.jpg',
--   preview_image_womens = 'personalisation/womens/blank-bottle.jpg',
--   preview_image_unisex = 'personalisation/unisex/blank-bottle.jpg',
--   preview_image_diffuser = 'personalisation/diffuser/blank-bottle.jpg',
--   fee = 20,
--   updated_at = now()
-- where code = 'default';
--
-- Storefront step-1 category codes: mens | womens | unisex | diffuser
-- Pick preview with: preview_image_{category} (fallback: preview_image_url)
