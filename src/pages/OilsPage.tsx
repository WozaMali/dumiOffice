...
const OilsPage = () => {
  ...
  const handleScentProductsCsv = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      const normHeaderKey = (raw: string) =>
        stripQuotes(raw.trim())
          .replace(/^\ufeff/, "")
          .trim()
          .toLowerCase()
          .replace(/\s+/g, " ");
      ...
      const delimiter = headerLineRaw.includes("\t")
        ? "\t"
        : headerLineRaw.includes(";")
          ? ";"
          : ",";
      const headers = headerLineRaw.split(delimiter).map((h) => normHeaderKey(h));

      const idx = (name: string) => headers.indexOf(normHeaderKey(name));

      const firstMatchingCol = (...candidates: string[]) => {
      ...
      // UI column is "Dumi Essence"; many sheets use that header instead of "brand".
      const iBrand = firstMatchingCol(
        "dumi essence",
        "dumi essence name",
        "brand",
      );
      const iItem = idx("item");
      const iInspired = firstMatchingCol("inspired by", "inspiredby");
      const iDesigner = firstMatchingCol("designer", "designer / house", "designer/house");
